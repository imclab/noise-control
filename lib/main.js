...
prefService.set("media.defaultAudioChannel", "content");
prefService.set("media.useAudioChannelService", true);
...
let listener = {
	...
	receiveMessage: function(message) {
		...
		case "NoiseControl:unloaded":
			...
			break;
		case "NoiseControl:volume":
			setIndicatorVolume(indicator, message.data);
			break;
				...
				for (let xulTab of chromeWindow.gBrowser.tabs) {
					if (xulTab.getAttribute("linkedpanel") == data.id) {
						...
						if (typeof data.state == "boolean") {
							...
							setIndicatorVolume(indicator, data.state);
						}
						chromeWindow.gBrowser.getBrowserForTab(xulTab)
							.messageManager.sendAsyncMessage("NoiseControl:setAudioState", data.state);
						return;
					}
				}
			}
...
exports.onUnload = function(reason) {
	...
	prefService.reset("media.defaultAudioChannel", "content");
	prefService.reset("media.useAudioChannelService");
...
function getIndicatorForTab(xulTab) {
	...
	if (!indicator) {
		...
		indicator.addEventListener("mousedown", function(event) {
			...
			chromeWindow.gBrowser.getBrowserForTab(xulTab).messageManager.sendAsyncMessage("NoiseControl:setAudioState", muted);
...
function setIndicatorVolume(indicator, volume = 1) {
	indicator.setAttribute("volume", volume);
	if (volume > 0.66) {
		indicator.setAttribute("level", "high");
	} else if (volume > 0.33) {
		indicator.setAttribute("level", "medium");
	} else if (volume > 0) {
		indicator.setAttribute("level", "low");
	} else {
		indicator.setAttribute("level", "off");
	}
}
...
function getNoisyTabData(xulTab, indicator) {
	return {
		...
		volume: indicator.hasAttribute("volume") ? parseFloat(indicator.getAttribute("volume"), 10) : 1
	};
}
